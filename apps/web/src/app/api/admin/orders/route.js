...
export async function PATCH(req) {
    try {
        ...
        switch (action) {
            ...
            case 'ADD_NOTES':
                if (!notes) {
                    return NextResponse.json({ error: 'Notes are required for add notes action' }, { status: 400 });
                }
                for (const orderId of orderIds) {
                    const updatedOrder = await prisma.order.update({
                        where: { id: orderId },
            ...
            case 'REFUND':
                for (const orderId of orderIds) {
                    ...
                    if (order.paymentStatus !== 'PAID') {
                        return NextResponse.json({ error: `Order ${orderId} is not paid and cannot be refunded` }, { status: 400 });
                    }
                    const updatedOrder = await prisma.order.update({
                        where: { id: orderId },
                        data: {
